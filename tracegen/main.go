...
const template = `
	...
	{{ range $methodName, $method := . }}
		func (t *{{$structName}}) {{$methodName}}{{signature $method}} {
			span, ctx := tracing.StartSpanFromContext(ctx, t.prefix + ".{{$interfaceName}}.{{$methodName}}")
			defer span.Finish()
...
func main() {
	...
	gen := NewDecoratorGenerator(template, []string{"github.com/teambition/gear-tracing"}, []validatorFunc{fn})
